...
//debug
    ...
    if (ssl->cert_pem_data) {
        ...
        mbedtls_x509_crt_free(&ssl->owncert);
        mbedtls_x509_crt_init(&ssl->owncert);

    /* Convert to an mbedtls key */
    if (0 != atca_mbedtls_pk_init(&ssl->pkey, 0))
    {
    ...
    if ((ret = atca_mbedtls_cert_add(&ssl->owncert, &g_cert_def_2_device)) != 0)
    {
        printf("Failed to parse device cert from device:%d\n", ret);
        goto exit;
    }
    ...
    if (0 != atca_mbedtls_cert_add(&ssl->owncert, &g_cert_def_1_signer))
    {
    ...
    printf(" ok\n");
        ...
        printf("MBEDTLS_SSL_VERIFY_REQUIRED\n");
