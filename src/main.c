...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            msg_id = esp_mqtt_client_subscribe(client, (const char *)subtopic, 1);
        ...
        case MQTT_EVENT_SUBSCRIBED:
            ...
            msg_id = esp_mqtt_client_publish(client, (const char *)pubtopic, "data", 0, 1, 0);
...
static void mqtt_app_start(void)
{
    const esp_mqtt_client_config_t mqtt_cfg = {
        .uri = (const char *)azureioturl,
        .client_id = (const char *)serialstr,
        .username = (const char *)usrname,
...
esp_mqtt_client_publish(client, pubtopic, pubMessage, 0, 1, 0);

//const portTickType yDelay = 3600000 / portTICK_RATE_MS; // 1000ms, 1hour
const portTickType yDelay = 50000 / portTICK_RATE_MS;
vTaskDelay(yDelay);
...
void sysinit(void)
{
   ...
   char character[2] = {};
   char tmpserial[ATCA_SERIAL_NUM_SIZE *2] = {};
	for ( int i = 0; i< ATCA_SERIAL_NUM_SIZE; i++){
        sprintf(character , "%02x", serial[i]);
	    strcat(tmpserial, character);
        }

printf("Serial Number Strings%s", tmpserial);
    strcpy(serialstr , tmpserial);


    sprintf(subtopic, "devices/%s/messages/devicebound/#", tmpserial);
    sprintf(pubtopic, "devices/%s/messages/events/", tmpserial);
    ...
    sprintf(usrname, "%s/%s/?api-version=2018-06-30", urlread , serialstr);
    sprintf(azureioturl, "mqtts://%s:8883", urlread);
